# iqdb-eval -- Engineering Directives
...
iqdb-eval is the evaluation harness: standard datasets, correct recall@k, latency percentiles, and throughput. Vector databases live and die by benchmarks, and this crate makes iQDB's numbers reproducible. It is generic over the `Index` trait and uses `iqdb-flat` to generate ground truth when none is provided.
...
- `iqdb-flat`: ground-truth generation
